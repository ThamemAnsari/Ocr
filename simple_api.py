...
def require_auth(request: Request) -> User:
    ...
    if session_info.get("expires_at") < datetime.now():
...
async def find_authentik_user(username: str) -> Optional[Dict]:
    ...
    try:
        ...
        print(f"[AUTHENTIK] Error finding user: {e}")
        return None
...
@app.get("/auth/me", response_model=Optional[User])
...
async def get_active_sessions(current_user: Optional[User] = Depends(get_current_user_optional)):
    ...
    try:
        ...
        for idx, record in enumerate(records_to_sync, 1):
            ...
            try:
                ...
                # Check if format function returned None
                if not zoho_record:
                ...
                if request.sync_mode == 'auto':
                    ...
                    if scholar_id_field:
                        ...
                        check_response = requests.get(check_url, headers=headers, params={'criteria': f'Scholar_ID == "{scholar_id_field}"', 'max_records': 1}, timeout=10)
