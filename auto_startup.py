...
# Internal API base (same process – call ourselves via 127.0.0.1 to avoid DNS issues)
_API_BASE = os.getenv("VITE_API_URL", "http://127.0.0.1:8000")
...
def _trigger_auto_extract():
    ...
    try:
        # Retry with exponential backoff if there are connection issues
        retry_count = 0
        max_retries = 3
        start_resp = None
        ...
        while retry_count < max_retries:
            try:
                ...
                break
            except (requests.ConnectionError, OSError, requests.exceptions.Timeout) as conn_err:
                retry_count += 1
                wait_time = 2 ** retry_count  # exponential backoff: 2s, 4s, 8s
                if retry_count < max_retries:
                    print(f"[AUTO-STARTUP] Attempt {retry_count} failed, retrying in {wait_time}s...")
                    time.sleep(wait_time)
                else:
                    print(f"[AUTO-STARTUP] ⚠️  Could not reach API after {max_retries} attempts. Extraction will need to be triggered manually.")
                    startup_job_state["status"] = "failed"
                    startup_job_state["message"] = "Could not reach API. Try triggering extraction manually."
                    return

        if start_resp and start_data.get("success"):
            ...
            if "already been extracted" in err or "All selected" in err:
                ...
                print(f"[AUTO-STARTUP] ✅ All records already extracted")
            else:
                startup_job_state["status"]  = "failed"
                startup_job_state["message"] = f"❌ {err}"
                print(f"[AUTO-STARTUP] ⚠️  {err}")

    except Exception as exc:
        startup_job_state["status"]  = "idle"  # Reset to idle instead of failed
        startup_job_state["message"] = "Auto-extract skipped. Use frontend to trigger manually."
        print(f"[AUTO-STARTUP] ⚠️  Startup extraction skipped ({type(exc).__name__}). You can trigger it manually from the frontend.")
